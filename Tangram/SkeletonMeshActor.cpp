	...
	: mesh(mesh), skeletonMeshRender(mesh, material), Actor(name)
{
	...
	for (int i = 0; i < bones.size(); i++) {
		...
		t = tm;
		bones[i]->setPosition(t.translation());
		bones[i]->setRotation(Quaternionf(t.rotation()));
		bones[i]->setScale({ tm.block(0, 0, 3, 1).norm(), tm.block(0, 1, 3, 1).norm(), tm.block(0, 2, 3, 1).norm() });
...
bool SkeletonMeshActor::deserialize(const SerializationInfo & from)
{
	...
	const SerializationInfo* cinfo = from.get("boneCollisions");
	if (cinfo != NULL) {
		SkeletonPhysicalAsset asset;
		asset.deserialize(*cinfo);
		asset.apply(*this);
	}
...
bool SkeletonMeshActor::serialize(SerializationInfo & to) {
	...
	SerializationInfo* cinfo = to.add("boneCollisions");
	SkeletonPhysicalAsset asset;
	asset.collect(*this);
