...
{
	transformUpdate = this->spawnTransform != spawnTransform;
	...
	if (emitDuration == 0 || time <= emitDuration) {
		for (auto b = particles.begin(), e = particles.end(); b != e;) {
			...
			if (hasCollision) {
				...
				b->update(deltaTime);
				ContactInfo info;
				if (Engine::getCurrentWorld()->physicalWorld.rayTest(p.position, b->position, info)) {
					Vector3f dir0 = (p.position - b->position).normalized();
					Vector3f dir1 = (2 * dir0.dot(info.normal)) * info.normal - dir0;
					float damp = boundDamp.uniform();
					b->position = info.location + dir1 * ((b->position - info.location).norm() * damp);
					b->velocity = dir1 * (b->velocity.norm() * damp);
				}
			}
			else
				b->update(deltaTime);
