...
void Input::resetHandlers()
{
	...
	mouseMoveHandler.clear();
	mouseWheelInputHandler.clear();
...
void Input::mouseInput(int button, int state, int x, int y)
{
	...
	mouseInputHandler(this, (MouseButtonEnum)button, (InputStateEnum)state, x, y);
}

void Input::mouseMove(int x, int y)
{
	mouseMoveHandler(this, x, y);
}

void Input::mouseWheelInput(int button, int dir, int x, int y)
{
	mouseWheelVal = dir;
	mouseWheelInputHandler(this, dir, x, y);
}
...
void Input::keyboardInput(char key, int state)
{
	...
	keyboardInputHandler(this, key, (InputStateEnum)state);
}
...
void Input::tick(float deltaTime)
{
	if (GetActiveWindow() == _hwnd) {
		...
		if ((mouseWheelVal = dimouse.lZ / 120) != 0)
			mouseWheelInputHandler(this, mouseWheelVal, mousePos.x, mousePos.y);
