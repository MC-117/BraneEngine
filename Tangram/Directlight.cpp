...
void DirectLight::afterTick()
{
	...
	if (world != NULL) {
		...
		/*float halfy = tanf(shadowCamera.fov * PI / 360.0);
		float halfx = halfy * shadowCamera.aspect;
		Vector3f p[4] = {
			{ shadowCamera.zNear,  }
		};
		Vector3f dirSpace = { dirPos.dot(dirFW), dirPos.dot(dirRW), dirPos.dot(dirUW) };
		if (dirSpace.x()) {
...
void DirectLight::preRender()
{
	...
	directionIdx = Shader::getAttributeIndex(pid, "directLight.direction");
	if (directionIdx != -1)
		glProgramUniform3f(pid, directionIdx, forward.x(), forward.y(), forward.z());
	intensityIdx = Shader::getAttributeIndex(pid, "directLight.intensity");
	if (intensityIdx != -1)
		glProgramUniform1f(pid, intensityIdx, intensity);
	colorIdx = Shader::getAttributeIndex(pid, "directLight.color");
	if (colorIdx != -1)
		glProgramUniform3f(pid, colorIdx, color.r, color.g, color.b);
	/*ambientIdx = Shader::getAttributeIndex(pid, "ambientColor");
	if (ambientIdx != -1)
