...
MeshActorAssetInfo MeshActorAssetInfo::assetInfo;

MeshActorAssetInfo::MeshActorAssetInfo() : AgentInfo("MeshActor")
{
	dependences = { &MeshAssetInfo::getInstance(), &MaterialAssetInfo::getInstance() };
	properties = { "physicalMaterial", "collisionShape" };
}

void * MeshActorAssetInfo::load(const string& name, const string & path, const vector<string>& settings, const vector<void*>& dependences) const
{
	Mesh* m = (Mesh*)dependences[0];
	Material* mat = (Material*)dependences[1];
	PhysicalMaterial pmat;
	if (!toPhysicalMaterial(settings[0], pmat)) {
		cout << "MeshActor: Load Physical Material(" << settings[0].c_str() << ") failed\n";
		return NULL;
	}
	Shape* s = toGeometry(settings[1]);
	MeshActor* ma = new MeshActor(*m, *mat, pmat, *(s == NULL ? m : s), name);
	delete s;
	return ma;
}

void * MeshActorAssetInfo::instance(const void * src, vector<string> settings, const string & displayName) const
{

	return nullptr;
}

AssetInfo & MeshActorAssetInfo::getInstance()
{
	return assetInfo;
}
...
CollisionActorAssetInfo CollisionActorAssetInfo::assetInfo;

CollisionActorAssetInfo::CollisionActorAssetInfo() : AssetInfo("CollisionActor")
{
	properties = { "name", "physicalMaterial", "collisionShape" };
}

void * CollisionActorAssetInfo::load(const string& name, const string & path, const vector<string>& settings, const vector<void*>& dependences) const
{
	PhysicalMaterial pm;
	if (!toPhysicalMaterial(settings[0], pm)) {
		cout << "CollisionActorAssetInfo: Load Physical Material(" << settings[0].c_str() << ") failed\n";
		return NULL;
	}
	Shape* s = toGeometry(settings[1]);
	if (s == NULL) {
		cout << "CollisionActor: Load collisionShape failed\n";
		delete s;
		return NULL;
	}
	CollisionActor* ca = new CollisionActor(s, pm, name);
	delete s;
	return ca;
}

AssetInfo & CollisionActorAssetInfo::getInstance()
{
	return assetInfo;
}
