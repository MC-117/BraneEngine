...
PTransform toPTransform(const Vector3f & v, const Quaternionf & q)
{
	...
	return PTransform(toPQuat(q), toPVec3(v));
#endif
...
{
	experimental::filesystem::path p = pwd;
	p /= path;
...
bool readHeadFile(const string& codeLine, string& code, const string& envPath, unordered_set<string>& headFiles)
{
	...
	trim(filePath, " <>\"");
	string _filePath = validateRelativePath(getFilePath(filePath, envPath));
	if (_filePath.empty()) {
...
GLBuffer::GLBuffer(GLuint type, GLuint cellSize, GLuint size) : type(type), cellSize(cellSize), size(size)
{
	if (size != 0)
		resize(size);
}
...
GLuint GLBuffer::bindBase(GLuint index)
{
	if (id == 0)
		return 0;
...
GLuint GLBuffer::resize(GLuint size)
{
	if (size == 0) {
		capacity = 0;
		unbind();
...
GLuint GLBuffer::uploadSubData(GLuint first, GLuint size, void * data)
{
...
GLuint GLBuffer::uploadData(GLuint size, void * data)
{
	resize(size);
	bind();
